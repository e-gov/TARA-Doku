...
| `nbf` (_Not Before_)   | `1530267052` - sama, mis `iat` väärtus. Ei ole OpenID Connect standardijärgne väide, vaid tagasiühilduvuseks vanemate TARA versioonidega. Selle asemel on soovitav kasutada `iat` väärtust. |
...
| `state` | `abcdefghijklmnop` - turvaelement. Autentimispäringu `state` parameetri väärtus. Ei ole OpenID Connect standardijärgne väli, vaid tagasiühilduvuseks vanemate TARA versioonidega. Selle asemel on soovitav kasutada tagasisuunamispäringu `state` URL-parameetrit. |
...
| `at_hash` | `X0MVjwrmMQs/IBzfU2osvw==` - pääsutõendi räsi. TARA-s ei kasutata. Väärtus ei ole standardijärgse Base64 URL kodeeringuga, vaid tagasiühilduvuseks vanemate TARA versioonidega on Base64 Standard kodeeringuga. |
...
|-----------------|--------------|
| 1.27, 27.11.2023   | Eemaldatud autentimispäringu parameeter `locale`, mis pole toetatud pärast 2019. aasta juulit. Identsustõendis `nbf`, `state` ja `at_hash` ei ole standardijärgsed (säilitatakse tagasiühilduvuseks). `state` ja `nonce` parameetrite kasutuse täpsustused. |
